class Trie(){
  ...
  def prefixMatchingString0(s: String): Set[Int] = {
    var current = Option(root)
  ...
  def prefixMatchingString(s: String): Set[String] = {
    prefixMatchingString0(s).map(s.substring(0, _))
  }

  def stringMatchingPrefix(s: String): Set[String] = {
    var current = Option(root)
    for(c <- s if current.notEmpty) current = current.get.children.get(c)
